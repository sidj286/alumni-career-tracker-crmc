...
try {
    ...
    // Validate role
    if (!in_array($input['role'], ['admin', 'dean'])) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Invalid role specified']);
        exit();
    }

    // Validate department for dean role
    if ($input['role'] === 'dean' && (empty($input['department']))) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Department is required for Dean role']);
        exit();
    }
